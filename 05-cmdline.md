...
For example, we may want a program that reads a data set and prints the average inflammation per raptor:
...
With this in hand, let's build a version of `readings.R` that always prints the per-raptor (per-row) mean of a single data file.
...
main <- function() {
  ...
  mean_per_raptor <- apply(dat, 1, mean)
  cat(mean_per_raptor, sep = "\n")
}
...
Since 60 lines of output per file is a lot to page through, we'll start by using three smaller files, each of which has three days of data for two raptors.
...
Let's try it out.
Instead of calculating the mean inflammation of every raptor, we'll only calculate the mean for the first 10 raptors (rows):
